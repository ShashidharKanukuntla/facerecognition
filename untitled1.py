...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 14 07:14:40 2018
...
def detect(gray,frame,known_faces,face_names):
    ...
    for (x, y, w, h) in faces:
        ...
        prescode=face_recognition.face_encodings(frame[y:y+h,x:x+w])[0]
        if len(prescode)>0:
            print(len(prescode))
            name=getname(known_codes,prescode,face_names)
            cv2.putText(frame, name , (x-10, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        else :
            print('no face found')
            continue
    return frame
